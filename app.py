...
def participant_html():
    ...
    script = (
        ...
        "return d.innerHTML;}"
        "function load(){fetch('/state').then(r=>r.json()).then(render);}"
        "function render(st){S=st;var v=document.getElementById('view');"
        "var open=null;"
        ...
        "if(res.exercise)markSent(res.exercise);load();}});}"
